...
void main() {
  group('SQLite:', () {
    final config = SqliteConfig('laconic.db');
    ...
    test('table(users).pluck(key: "id") returns map', () async {
      var map = await laconic.table(userTable).pluck('name', key: 'id')
    ...
    test('join with whereNotNull condition', () async {
      var results = await laconic
          ...
          .join(
            '$postTable p',
            (join) => join.on('u.id', 'p.user_id').whereNotNull('p.content'),
          )
          .orderBy('u.name')
          .get();
      expect(results.isNotEmpty, isTrue);
    ...
    test('join with whereColumn condition', () async {
      await laconic.table(postTable).insert([
        {'user_id': 1, 'title': 'Special Post', 'content': 'Test'},
      ]);
      ...
      await laconic.table(postTable).where('title', 'Special Post').delete();
    });
